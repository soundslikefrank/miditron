EESchema Schematic File Version 4
EELAYER 30 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 1 7
Title ""
Date ""
Rev ""
Comp ""
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
$Sheet
S 7900 2600 1350 3700
U 61006ED6
F0 "front" 50
F1 "midi2cv-front.sch" 50
F2 "led_out_a_g" I L 7900 2900 50 
F3 "led_out_a_b" I L 7900 3050 50 
F4 "led_out_a_r" I L 7900 2750 50 
F5 "led_out_d_r" I L 7900 4400 50 
F6 "led_out_d_b" I L 7900 4700 50 
F7 "led_out_d_g" I L 7900 4550 50 
F8 "led_out_c_r" I L 7900 3850 50 
F9 "led_out_c_b" I L 7900 4150 50 
F10 "led_out_c_g" I L 7900 4000 50 
F11 "led_out_b_r" I L 7900 3300 50 
F12 "led_out_b_b" I L 7900 3600 50 
F13 "led_out_b_g" I L 7900 3450 50 
F14 "jack_detect" I R 9250 5450 50 
F15 "cv_in_a" O L 7900 5950 50 
F16 "cv_in_b" O L 7900 6100 50 
F17 "clock_out" I R 9250 5700 50 
F18 "run_out" I R 9250 5850 50 
F19 "1_16_out" I R 9250 6000 50 
F20 "var_out" I R 9250 6150 50 
F21 "dac4_out_a" I R 9250 2750 50 
F22 "dac4_out_b" I R 9250 2900 50 
F23 "dac4_out_c" I R 9250 3050 50 
F24 "dac4_out_d" I R 9250 3200 50 
F25 "gate_out_a" I R 9250 4750 50 
F26 "gate_out_b" I R 9250 4900 50 
F27 "gate_out_c" I R 9250 5050 50 
F28 "gate_out_d" I R 9250 5200 50 
F29 "dac8_out_a" I R 9250 3450 50 
F30 "dac8_out_b" I R 9250 3600 50 
F31 "dac8_out_c" I R 9250 3750 50 
F32 "dac8_out_d" I R 9250 3900 50 
F33 "dac8_out_e" I R 9250 4050 50 
F34 "dac8_out_f" I R 9250 4200 50 
F35 "dac8_out_g" I R 9250 4350 50 
F36 "dac8_out_h" I R 9250 4500 50 
F37 "button_a" O L 7900 4950 50 
F38 "button_b" O L 7900 5100 50 
F39 "button_d" O L 7900 5400 50 
F40 "button_c" O L 7900 5250 50 
F41 "midi_in+" O L 7900 5650 50 
F42 "midi_in-" O L 7900 5800 50 
$EndSheet
$Sheet
S 2650 2600 1350 3700
U 61005BD4
F0 "back" 50
F1 "midi2cv-back.sch" 50
F2 "led_out_a_r" O L 2650 2750 50 
F3 "led_out_d_b" O L 2650 4700 50 
F4 "led_out_d_g" O L 2650 4550 50 
F5 "led_out_d_r" O L 2650 4400 50 
F6 "led_out_c_b" O L 2650 4150 50 
F7 "led_out_c_g" O L 2650 4000 50 
F8 "led_out_c_r" O L 2650 3850 50 
F9 "led_out_b_b" O L 2650 3600 50 
F10 "led_out_b_g" O L 2650 3450 50 
F11 "led_out_b_r" O L 2650 3300 50 
F12 "led_out_a_b" O L 2650 3050 50 
F13 "led_out_a_g" O L 2650 2900 50 
F14 "dac4_out_a" O R 4000 2750 50 
F15 "dac4_out_b" O R 4000 2900 50 
F16 "dac4_out_c" O R 4000 3050 50 
F17 "dac4_out_d" O R 4000 3200 50 
F18 "dac8_out_a" O R 4000 3450 50 
F19 "dac8_out_b" O R 4000 3600 50 
F20 "dac8_out_c" O R 4000 3750 50 
F21 "dac8_out_d" O R 4000 3900 50 
F22 "dac8_out_e" O R 4000 4050 50 
F23 "dac8_out_f" O R 4000 4200 50 
F24 "dac8_out_g" O R 4000 4350 50 
F25 "dac8_out_h" O R 4000 4500 50 
F26 "midi_in+" I L 2650 5650 50 
F27 "button_a" I L 2650 4950 50 
F28 "button_b" I L 2650 5100 50 
F29 "button_c" I L 2650 5250 50 
F30 "button_d" I L 2650 5400 50 
F31 "jack_detect" O R 4000 5450 50 
F32 "gate_out_a" O R 4000 4750 50 
F33 "clock_out" O R 4000 5700 50 
F34 "run_out" O R 4000 5850 50 
F35 "1_16_out" O R 4000 6000 50 
F36 "gate_out_b" O R 4000 4900 50 
F37 "gate_out_c" O R 4000 5050 50 
F38 "gate_out_d" O R 4000 5200 50 
F39 "var_out" O R 4000 6150 50 
F40 "cv_jack_a" I L 2650 5950 50 
F41 "cv_jack_b" I L 2650 6100 50 
F42 "midi_in-" I L 2650 5800 50 
$EndSheet
Text GLabel 1650 800  0    50   Input ~ 0
led_out_a_r_back
$Comp
L power:+3.3V #PWR01
U 1 1 610CBBFD
P 1650 2150
F 0 "#PWR01" H 1650 2000 50  0001 C CNN
F 1 "+3.3V" H 1665 2323 50  0000 C CNN
F 2 "" H 1650 2150 50  0001 C CNN
F 3 "" H 1650 2150 50  0001 C CNN
	1    1650 2150
	-1   0    0    1   
$EndComp
$Comp
L power:GND #PWR02
U 1 1 610CD2D7
P 2150 2150
F 0 "#PWR02" H 2150 1900 50  0001 C CNN
F 1 "GND" H 2155 1977 50  0000 C CNN
F 2 "" H 2150 2150 50  0001 C CNN
F 3 "" H 2150 2150 50  0001 C CNN
	1    2150 2150
	1    0    0    -1  
$EndComp
Text GLabel 1650 900  0    50   Input ~ 0
led_out_a_b_back
Text GLabel 1650 1000 0    50   Input ~ 0
led_out_b_g_back
Text GLabel 1650 1100 0    50   Input ~ 0
led_out_c_r_back
Text GLabel 1650 1200 0    50   Input ~ 0
led_out_c_b_back
Text GLabel 1650 1300 0    50   Input ~ 0
led_out_d_g_back
Text GLabel 1650 1750 0    50   Input ~ 0
button_a_back
Text GLabel 1650 1850 0    50   Input ~ 0
button_c_back
Text GLabel 2150 2050 2    50   Input ~ 0
midi_in+_back
Text GLabel 2150 1650 2    50   Input ~ 0
cv_in_b_back
Text GLabel 2150 800  2    50   Input ~ 0
led_out_a_g_back
Text GLabel 2150 900  2    50   Input ~ 0
led_out_b_r_back
Text GLabel 2150 1000 2    50   Input ~ 0
led_out_b_b_back
Text GLabel 2150 1100 2    50   Input ~ 0
led_out_c_g_back
Text GLabel 2150 1200 2    50   Input ~ 0
led_out_d_r_back
Text GLabel 2150 1300 2    50   Input ~ 0
led_out_d_b_back
Text GLabel 2150 1750 2    50   Input ~ 0
button_b_back
Text GLabel 2150 1850 2    50   Input ~ 0
button_d_back
Text GLabel 1650 1650 0    50   Input ~ 0
cv_in_a_back
Text GLabel 3850 1400 0    50   Input ~ 0
gate_out_a_back
Text GLabel 3850 1600 0    50   Input ~ 0
gate_out_c_back
Text GLabel 1650 1950 0    50   Input ~ 0
jack_detect_back
Text GLabel 3850 1100 0    50   Input ~ 0
run_out_back
Text GLabel 3850 1300 0    50   Input ~ 0
var_out_back
$Comp
L power:GND #PWR04
U 1 1 610D386F
P 3850 900
F 0 "#PWR04" H 3850 650 50  0001 C CNN
F 1 "GND" H 3855 727 50  0000 C CNN
F 2 "" H 3850 900 50  0001 C CNN
F 3 "" H 3850 900 50  0001 C CNN
	1    3850 900 
	-1   0    0    1   
$EndComp
Text GLabel 3850 1500 0    50   Input ~ 0
gate_out_b_back
Text GLabel 3850 1700 0    50   Input ~ 0
gate_out_d_back
Text GLabel 3850 1000 0    50   Input ~ 0
clock_out_back
Text GLabel 3850 1200 0    50   Input ~ 0
1_16_out_back
Text GLabel 2650 2750 0    50   Input ~ 0
led_out_a_r_back
Text GLabel 2650 3050 0    50   Input ~ 0
led_out_a_b_back
Text GLabel 2650 3450 0    50   Input ~ 0
led_out_b_g_back
Text GLabel 2650 3850 0    50   Input ~ 0
led_out_c_r_back
Text GLabel 2650 4150 0    50   Input ~ 0
led_out_c_b_back
Text GLabel 2650 4550 0    50   Input ~ 0
led_out_d_g_back
Text GLabel 2650 4950 0    50   Input ~ 0
button_a_back
Text GLabel 2650 5250 0    50   Input ~ 0
button_c_back
Text GLabel 2650 5650 0    50   Input ~ 0
midi_in+_back
Text GLabel 2650 6100 0    50   Input ~ 0
cv_in_b_back
Text GLabel 2650 2900 0    50   Input ~ 0
led_out_a_g_back
Text GLabel 2650 3300 0    50   Input ~ 0
led_out_b_r_back
Text GLabel 2650 3600 0    50   Input ~ 0
led_out_b_b_back
Text GLabel 2650 4000 0    50   Input ~ 0
led_out_c_g_back
Text GLabel 2650 4400 0    50   Input ~ 0
led_out_d_r_back
Text GLabel 2650 4700 0    50   Input ~ 0
led_out_d_b_back
Text GLabel 2650 5100 0    50   Input ~ 0
button_b_back
Text GLabel 2650 5400 0    50   Input ~ 0
button_d_back
Text GLabel 2650 5950 0    50   Input ~ 0
cv_in_a_back
Text GLabel 4000 2750 2    50   Input ~ 0
dac4_out_a_back
Text GLabel 4000 3050 2    50   Input ~ 0
dac4_out_c_back
Text GLabel 4000 3450 2    50   Input ~ 0
dac8_out_a_back
Text GLabel 4000 3750 2    50   Input ~ 0
dac8_out_c_back
Text GLabel 4000 4050 2    50   Input ~ 0
dac8_out_e_back
Text GLabel 4000 4350 2    50   Input ~ 0
dac8_out_g_back
Text GLabel 4000 4750 2    50   Input ~ 0
gate_out_a_back
Text GLabel 4000 5050 2    50   Input ~ 0
gate_out_c_back
Text GLabel 4000 5450 2    50   Input ~ 0
jack_detect_back
Text GLabel 4000 5850 2    50   Input ~ 0
run_out_back
Text GLabel 4000 6150 2    50   Input ~ 0
var_out_back
Text GLabel 4000 2900 2    50   Input ~ 0
dac4_out_b_back
Text GLabel 4000 3200 2    50   Input ~ 0
dac4_out_d_back
Text GLabel 4000 3600 2    50   Input ~ 0
dac8_out_b_back
Text GLabel 4000 3900 2    50   Input ~ 0
dac8_out_d_back
Text GLabel 4000 4200 2    50   Input ~ 0
dac8_out_f_back
Text GLabel 4000 4500 2    50   Input ~ 0
dac8_out_h_back
Text GLabel 4000 4900 2    50   Input ~ 0
gate_out_b_back
Text GLabel 4000 5200 2    50   Input ~ 0
gate_out_d_back
Text GLabel 4000 5700 2    50   Input ~ 0
clock_out_back
Text GLabel 4000 6000 2    50   Input ~ 0
1_16_out_back
Text GLabel 9250 2750 2    50   Input ~ 0
dac4_out_a_front
Text GLabel 9250 3050 2    50   Input ~ 0
dac4_out_c_front
Text GLabel 9250 3450 2    50   Input ~ 0
dac8_out_a_front
Text GLabel 9250 3750 2    50   Input ~ 0
dac8_out_c_front
Text GLabel 9250 4050 2    50   Input ~ 0
dac8_out_e_front
Text GLabel 9250 4350 2    50   Input ~ 0
dac8_out_g_front
Text GLabel 9250 4750 2    50   Input ~ 0
gate_out_a_front
Text GLabel 9250 5050 2    50   Input ~ 0
gate_out_c_front
Text GLabel 9250 5450 2    50   Input ~ 0
jack_detect_front
Text GLabel 9250 5850 2    50   Input ~ 0
run_out_front
Text GLabel 9250 6150 2    50   Input ~ 0
var_out_front
Text GLabel 9250 2900 2    50   Input ~ 0
dac4_out_b_front
Text GLabel 9250 3200 2    50   Input ~ 0
dac4_out_d_front
Text GLabel 9250 3600 2    50   Input ~ 0
dac8_out_b_front
Text GLabel 9250 3900 2    50   Input ~ 0
dac8_out_d_front
Text GLabel 9250 4200 2    50   Input ~ 0
dac8_out_f_front
Text GLabel 9250 4500 2    50   Input ~ 0
dac8_out_h_front
Text GLabel 9250 4900 2    50   Input ~ 0
gate_out_b_front
Text GLabel 9250 5200 2    50   Input ~ 0
gate_out_d_front
Text GLabel 9250 5700 2    50   Input ~ 0
clock_out_front
Text GLabel 9250 6000 2    50   Input ~ 0
1_16_out_front
Text GLabel 7900 2750 0    50   Input ~ 0
led_out_a_r_front
Text GLabel 7900 3050 0    50   Input ~ 0
led_out_a_b_front
Text GLabel 7900 3450 0    50   Input ~ 0
led_out_b_g_front
Text GLabel 7900 3850 0    50   Input ~ 0
led_out_c_r_front
Text GLabel 7900 4150 0    50   Input ~ 0
led_out_c_b_front
Text GLabel 7900 4550 0    50   Input ~ 0
led_out_d_g_front
Text GLabel 7900 4950 0    50   Input ~ 0
button_a_front
Text GLabel 7900 5250 0    50   Input ~ 0
button_c_front
Text GLabel 7900 5650 0    50   Input ~ 0
midi_in+_front
Text GLabel 7900 6100 0    50   Input ~ 0
cv_in_b_front
Text GLabel 7900 2900 0    50   Input ~ 0
led_out_a_g_front
Text GLabel 7900 3300 0    50   Input ~ 0
led_out_b_r_front
Text GLabel 7900 3600 0    50   Input ~ 0
led_out_b_b_front
Text GLabel 7900 4000 0    50   Input ~ 0
led_out_c_g_front
Text GLabel 7900 4400 0    50   Input ~ 0
led_out_d_r_front
Text GLabel 7900 4700 0    50   Input ~ 0
led_out_d_b_front
Text GLabel 7900 5100 0    50   Input ~ 0
button_b_front
Text GLabel 7900 5400 0    50   Input ~ 0
button_d_front
Text GLabel 7900 5950 0    50   Input ~ 0
cv_in_a_front
Text GLabel 6900 800  0    50   Input ~ 0
led_out_a_r_front
$Comp
L power:+3.3V #PWR06
U 1 1 61139899
P 6900 2150
F 0 "#PWR06" H 6900 2000 50  0001 C CNN
F 1 "+3.3V" H 6915 2323 50  0000 C CNN
F 2 "" H 6900 2150 50  0001 C CNN
F 3 "" H 6900 2150 50  0001 C CNN
	1    6900 2150
	-1   0    0    1   
$EndComp
$Comp
L power:GND #PWR07
U 1 1 6113989F
P 7400 2150
F 0 "#PWR07" H 7400 1900 50  0001 C CNN
F 1 "GND" H 7405 1977 50  0000 C CNN
F 2 "" H 7400 2150 50  0001 C CNN
F 3 "" H 7400 2150 50  0001 C CNN
	1    7400 2150
	1    0    0    -1  
$EndComp
Text GLabel 6900 900  0    50   Input ~ 0
led_out_a_b_front
Text GLabel 6900 1000 0    50   Input ~ 0
led_out_b_g_front
Text GLabel 6900 1100 0    50   Input ~ 0
led_out_c_r_front
Text GLabel 6900 1200 0    50   Input ~ 0
led_out_c_b_front
Text GLabel 6900 1300 0    50   Input ~ 0
led_out_d_g_front
Text GLabel 6900 1750 0    50   Input ~ 0
button_a_front
Text GLabel 6900 1850 0    50   Input ~ 0
button_c_front
Text GLabel 7400 1650 2    50   Input ~ 0
cv_in_b_front
Text GLabel 7400 800  2    50   Input ~ 0
led_out_a_g_front
Text GLabel 7400 900  2    50   Input ~ 0
led_out_b_r_front
Text GLabel 7400 1000 2    50   Input ~ 0
led_out_b_b_front
Text GLabel 7400 1100 2    50   Input ~ 0
led_out_c_g_front
Text GLabel 7400 1200 2    50   Input ~ 0
led_out_d_r_front
Text GLabel 7400 1300 2    50   Input ~ 0
led_out_d_b_front
Text GLabel 7400 1750 2    50   Input ~ 0
button_b_front
Text GLabel 7400 1850 2    50   Input ~ 0
button_d_front
Text GLabel 6900 1650 0    50   Input ~ 0
cv_in_a_front
Text GLabel 10700 1900 0    50   Input ~ 0
dac4_out_a_front
Text GLabel 10700 1500 0    50   Input ~ 0
dac8_out_a_front
Text GLabel 10700 900  0    50   Input ~ 0
dac8_out_e_front
Text GLabel 6900 1950 0    50   Input ~ 0
jack_detect_front
$Comp
L power:GND #PWR010
U 1 1 611398CB
P 9250 1750
F 0 "#PWR010" H 9250 1500 50  0001 C CNN
F 1 "GND" H 9255 1577 50  0000 C CNN
F 2 "" H 9250 1750 50  0001 C CNN
F 3 "" H 9250 1750 50  0001 C CNN
	1    9250 1750
	1    0    0    -1  
$EndComp
Text GLabel 10700 1600 0    50   Input ~ 0
dac4_out_d_front
Text GLabel 10700 1100 0    50   Input ~ 0
dac8_out_d_front
Text GLabel 10700 1300 0    50   Input ~ 0
dac8_out_h_front
Text GLabel 9250 1450 0    50   Input ~ 0
gate_out_b_front
Text GLabel 9250 1650 0    50   Input ~ 0
gate_out_d_front
Text GLabel 9250 950  0    50   Input ~ 0
clock_out_front
Text GLabel 9250 1150 0    50   Input ~ 0
1_16_out_front
Text GLabel 1650 2050 0    50   Input ~ 0
midi_in-_back
Text GLabel 7400 2050 2    50   Input ~ 0
midi_in+_front
Text GLabel 6900 2050 0    50   Input ~ 0
midi_in-_front
Text GLabel 2650 5800 0    50   Input ~ 0
midi_in-_back
Text GLabel 7900 5800 0    50   Input ~ 0
midi_in-_front
$Comp
L power:GND #PWR09
U 1 1 613C18F0
P 9250 850
F 0 "#PWR09" H 9250 600 50  0001 C CNN
F 1 "GND" H 9255 677 50  0000 C CNN
F 2 "" H 9250 850 50  0001 C CNN
F 3 "" H 9250 850 50  0001 C CNN
	1    9250 850 
	-1   0    0    1   
$EndComp
$Comp
L power:GND #PWR05
U 1 1 613C1D3B
P 3850 1800
F 0 "#PWR05" H 3850 1550 50  0001 C CNN
F 1 "GND" H 3855 1627 50  0000 C CNN
F 2 "" H 3850 1800 50  0001 C CNN
F 3 "" H 3850 1800 50  0001 C CNN
	1    3850 1800
	1    0    0    -1  
$EndComp
$Comp
L Connector_Generic:Conn_02x06_Odd_Even J1
U 1 1 6140B056
P 1850 1000
F 0 "J1" H 1900 1417 50  0000 C CNN
F 1 "Conn_02x06_Odd_Even" H 1900 1326 50  0000 C CNN
F 2 "Connector_PinSocket_2.54mm:PinSocket_2x06_P2.54mm_Vertical" H 1850 1000 50  0001 C CNN
F 3 "~" H 1850 1000 50  0001 C CNN
	1    1850 1000
	1    0    0    -1  
$EndComp
$Comp
L Connector_Generic:Conn_02x06_Odd_Even J2
U 1 1 6140D05F
P 1850 1850
F 0 "J2" H 1900 2267 50  0000 C CNN
F 1 "Conn_02x06_Odd_Even" H 1900 2176 50  0000 C CNN
F 2 "Connector_PinSocket_2.54mm:PinSocket_2x06_P2.54mm_Vertical" H 1850 1850 50  0001 C CNN
F 3 "~" H 1850 1850 50  0001 C CNN
	1    1850 1850
	1    0    0    -1  
$EndComp
$Comp
L Connector_Generic:Conn_02x06_Odd_Even J5
U 1 1 6140E179
P 7100 1000
F 0 "J5" H 7150 1417 50  0000 C CNN
F 1 "Conn_02x06_Odd_Even" H 7150 1326 50  0000 C CNN
F 2 "Connector_PinHeader_2.54mm:PinHeader_2x06_P2.54mm_Vertical" H 7100 1000 50  0001 C CNN
F 3 "~" H 7100 1000 50  0001 C CNN
	1    7100 1000
	1    0    0    -1  
$EndComp
$Comp
L Connector_Generic:Conn_02x06_Odd_Even J6
U 1 1 614101BB
P 7100 1850
F 0 "J6" H 7150 2267 50  0000 C CNN
F 1 "Conn_02x06_Odd_Even" H 7150 2176 50  0000 C CNN
F 2 "Connector_PinHeader_2.54mm:PinHeader_2x06_P2.54mm_Vertical" H 7100 1850 50  0001 C CNN
F 3 "~" H 7100 1850 50  0001 C CNN
	1    7100 1850
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR08
U 1 1 6157F15E
P 8050 1950
F 0 "#PWR08" H 8050 1700 50  0001 C CNN
F 1 "GND" H 8055 1777 50  0000 C CNN
F 2 "" H 8050 1950 50  0001 C CNN
F 3 "" H 8050 1950 50  0001 C CNN
	1    8050 1950
	0    -1   -1   0   
$EndComp
Wire Wire Line
	7400 1950 8050 1950
$Comp
L power:GND #PWR03
U 1 1 61581CE3
P 2850 1950
F 0 "#PWR03" H 2850 1700 50  0001 C CNN
F 1 "GND" H 2855 1777 50  0000 C CNN
F 2 "" H 2850 1950 50  0001 C CNN
F 3 "" H 2850 1950 50  0001 C CNN
	1    2850 1950
	0    -1   -1   0   
$EndComp
Wire Wire Line
	2150 1950 2850 1950
$Comp
L Connector_Generic:Conn_01x10 J3
U 1 1 615F1A61
P 4050 1300
F 0 "J3" H 4130 1292 50  0000 L CNN
F 1 "Conn_01x10" H 4130 1201 50  0000 L CNN
F 2 "Connector_PinSocket_2.54mm:PinSocket_1x10_P2.54mm_Vertical" H 4050 1300 50  0001 C CNN
F 3 "~" H 4050 1300 50  0001 C CNN
	1    4050 1300
	1    0    0    -1  
$EndComp
$Comp
L Connector_Generic:Conn_01x12 J4
U 1 1 6160A34B
P 5650 1350
F 0 "J4" H 5730 1342 50  0000 L CNN
F 1 "Conn_01x12" H 5730 1251 50  0000 L CNN
F 2 "Connector_PinSocket_2.54mm:PinSocket_1x12_P2.54mm_Vertical" H 5650 1350 50  0001 C CNN
F 3 "~" H 5650 1350 50  0001 C CNN
	1    5650 1350
	1    0    0    -1  
$EndComp
$Comp
L Connector_Generic:Conn_01x12 J8
U 1 1 615F8D47
P 10900 1300
F 0 "J8" H 10980 1292 50  0000 L CNN
F 1 "Conn_01x12" H 10980 1201 50  0000 L CNN
F 2 "Connector_PinHeader_2.54mm:PinHeader_1x12_P2.54mm_Vertical" H 10900 1300 50  0001 C CNN
F 3 "~" H 10900 1300 50  0001 C CNN
	1    10900 1300
	1    0    0    -1  
$EndComp
$Comp
L Connector_Generic:Conn_01x10 J7
U 1 1 61605EB6
P 9450 1250
F 0 "J7" H 9530 1242 50  0000 L CNN
F 1 "Conn_01x10" H 9530 1151 50  0000 L CNN
F 2 "Connector_PinHeader_2.54mm:PinHeader_1x10_P2.54mm_Vertical" H 9450 1250 50  0001 C CNN
F 3 "~" H 9450 1250 50  0001 C CNN
	1    9450 1250
	1    0    0    -1  
$EndComp
Text GLabel 10700 1000 0    50   Input ~ 0
dac8_out_f_front
Text GLabel 10700 1200 0    50   Input ~ 0
dac8_out_b_front
Text GLabel 10700 1800 0    50   Input ~ 0
dac4_out_b_front
Text GLabel 9250 1250 0    50   Input ~ 0
var_out_front
Text GLabel 9250 1050 0    50   Input ~ 0
run_out_front
Text GLabel 9250 1550 0    50   Input ~ 0
gate_out_c_front
Text GLabel 9250 1350 0    50   Input ~ 0
gate_out_a_front
Text GLabel 10700 1400 0    50   Input ~ 0
dac8_out_g_front
Text GLabel 10700 800  0    50   Input ~ 0
dac8_out_c_front
Text GLabel 10700 1700 0    50   Input ~ 0
dac4_out_c_front
Text GLabel 5450 1350 0    50   Input ~ 0
dac8_out_h_back
Text GLabel 5450 1050 0    50   Input ~ 0
dac8_out_f_back
Text GLabel 5450 1150 0    50   Input ~ 0
dac8_out_d_back
Text GLabel 5450 1250 0    50   Input ~ 0
dac8_out_b_back
Text GLabel 5450 1850 0    50   Input ~ 0
dac4_out_b_back
Text GLabel 5450 1450 0    50   Input ~ 0
dac8_out_g_back
Text GLabel 5450 950  0    50   Input ~ 0
dac8_out_e_back
Text GLabel 5450 850  0    50   Input ~ 0
dac8_out_c_back
Text GLabel 5450 1550 0    50   Input ~ 0
dac8_out_a_back
Text GLabel 5450 1750 0    50   Input ~ 0
dac4_out_c_back
Text GLabel 5450 1650 0    50   Input ~ 0
dac4_out_d_back
Text GLabel 5450 1950 0    50   Input ~ 0
dac4_out_a_back
$EndSCHEMATC
